...
| 版本 | 日期 | 变更点 |
|---|---|---|
| （未编号） | 2026-08-05 | 初版：变量定义表、跨上下文隔离矩阵、联调现状（examiner 14/14、tool 14/14、evaluation 16/18、outline 13/13） |
| v0.2 | 2026-08-25 | 顶部加版本号；第 5 节加注：模型按 purpose 分流，但变量注入规则不变；第 7 节联调现状保留为历史快照（详见 memory: prompts-regression-2026-08-07） |
...
> **v0.2 注**：模型已按 purpose 分流（对话/工具用 `LLM_MODEL_DIALOG` 速度优先，评估/题纲用 `LLM_MODEL_EVAL` 能力优先），但**变量注入规则不变**——分流只影响模型选择与 thinking 开关，不影响各上下文允许/禁止的变量集合。规则仍以本表为准。
