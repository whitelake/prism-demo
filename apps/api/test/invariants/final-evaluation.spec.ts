...
import { normalizeLevel } from '@/assessment/consistency';
...
interface MinimalEvalResponse {
  ...
  overall: {
    level: 'L0' | 'L1' | 'L2' | 'L3' | 'L4' | 'L3_pending' | 'L4_pending';
    track: '个人深度轨道' | '团队负责人轨道' | '无法判断';
    confidence: number;
    reasoning: string;
...
describe('终判 C 接入状态机 (e2e, mysql, mock LLM)', () => {
  ...
  describe('终判成功 → FINAL_EVALUATING → COMPLETED (PoC 不变量 5)', () => {
    ...
    beforeAll(async () => {
      const r = await seedForFinalEval({ levelA: 'L3_pending' });
      assessmentId = r.id;
      service = r.service;

      // 用 mock 替换 LlmClient.call：返回一个固定 level=L3 的合法响应
      ...
      (llmClient as unknown as { call: typeof llmClient.call }).call = (async () => {
        return {
          raw: JSON.stringify(makeFakeEvalResponse('L3')),
          parsed: makeFakeEvalResponse('L3') as unknown as never,
          logId: 'mock-log-id',
          model: 'mock-model',
          latencyMs: 10,
        };
    ...
    it('runFinalEvaluation 推进至 COMPLETED 并落库 evaluation(C) + consistency', async () => {
      ...
      expect(cons!.levelB).toBe('L3');
      expect(cons!.levelC).toBe('L3');
      // L3_pending 归一为 3，L3 也是 3 → aEqC=true, gap=0
      const aVal = normalizeLevel('L3_pending')!;
      const cVal = normalizeLevel('L3')!;
      const bVal = normalizeLevel('L3')!;
